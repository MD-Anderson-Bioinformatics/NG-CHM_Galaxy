...
#=========================
#DOCKER IZED  Big Query to NGCHM data via ISB API
#========================================
		...
		for k in range(3):
			...
			if tableType == 'mRNA_BCGSC_HiSeq_RPKM':
				if k == 0:
					...
					outfile.write( 'Gene\tSupplied Samples Average RPKM\tStudy Cohort Average\tStudy Cohort RPKM StdDev\tStudy '+study+'\tDB Table '+tableType+'\n' )
...
#				 if stat_type == 'STDDEV_SAMP':
				if k == 0:
					...
					outfile.write( 'Gene\tSupplied Samples Average Normalized Value\tStudy Cohort Average NV\tStudy Cohort NV StdDev\tStudy '+study+'\tDB Table '+tableType+'\n' )
				...
				for row in query_response['rows']:
					...
					if gene in geneSDAvginfo:
						...
						if(float(NV) < lowerBound  or float(NV) > upperBound ):
							...
							outfile.write( gene+'\t'+"%0.2f" % float(NV)+'\t'+"%0.2f" % float(geneSDAvginfo[gene][0])+'\t'+"%0.2f" % float(geneSDAvginfo[gene][1])+'\n' )
