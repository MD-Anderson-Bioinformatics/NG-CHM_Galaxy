...
#v7 add more selection criteria based on ISB  july2016 BETA release new BioSpecimen Table
...
#v7  new table query of Biospeciment table entries for min max 
#        'bio_lympho'>Lymphocyte infiltration percent
#         'bio_mono'>Monocyte infiltration percent
#         'bio_necro'>Necrosis percent
#         'bio_neut'>Neutrophil infiltration percent
#         'bio_norm'>Normal infiltration percent
#         'bio_strom'>Stromal infiltration percent
#         'bio_tumor'>Tumor infiltration percent
#         'bio_tumnuc'>Tumor nuclei infiltration percent
...
#                     elif (str(param[i]) != 'cat_EMPTY' and    str(param[i+1]) != 'Click cursor in middle of this text to see choices' and     str(param[i+1]) != '' ):
                    ...
                    noBio=  False
                    bioQueryString = bioQueryString+ ' BS.min_' +param[i][4:] +'>='+ (param[i+1]) + ' AND  BS.max_' +param[i][4:]+ '<= '+ (param[i+2]) +' AND '
                    print 'Found bio NUMBER param ', param[i], param[i+1],param[i+2]
                    i= i+3
        #end while
            if not error:                     
                queryString = queryString[0:-4]         
                bioQueryString = bioQueryString[0:-4]         
                saveLen = len(queryString)+ len(bioQueryString) 
...
#                    tempString = 'SELECT  PA.ParticipantBarcode FROM [isb-cgc:tcga_201607_beta.Clinical_data] as PA JOIN [isb-cgc:tcga_201607_beta.Biospecimen_data] as BS '
                ...
                if saveLen > 0 and noBio:  #no biospecimen params for query
                    queryString = tempString+ ' AND ' + queryString + ' );'           
                elif saveLen > 0 and len(queryString) <1:               
                    queryString = tempString+ ' AND ' + bioQueryString+ ');'           
                elif saveLen > 0:               
                    queryString = tempString+ ' AND ' + queryString + ' AND '+ bioQueryString+ ');'           
                else: 
